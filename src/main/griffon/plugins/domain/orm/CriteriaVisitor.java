    ...
    public void visitBinaryExpression(BinaryExpression expression) {
        ...
        if (lhs instanceof Criterion) {
            ...
            if (rhs instanceof Property) {
                current = new griffon.plugins.domain.orm.PropertyExpression(p.value, op, ((Property) rhs).value);
                return;
            } else if (rhs instanceof Value) {
                Value v = (Value) rhs;
                if ("null".equals(v.value) || v.value == null) {
                    current = new griffon.plugins.domain.orm.UnaryExpression(p.value, op == Operator.EQUAL ? Operator.IS_NULL : Operator.IS_NOT_NULL);
                } else {
                    current = new griffon.plugins.domain.orm.BinaryExpression(p.value, op, v.value);
                }
                return;
            ...
            if (rhs instanceof Property) {
                ...
                if ("null".equals(v.value) || v.value == null) {
                    ...
                    current = Restrictions.not(new griffon.plugins.domain.orm.BinaryExpression(p.value, op, v.value));
                }
                return;
            } else if (rhs instanceof Value) {
                throw new CriteriaVisitException(
